...
const CastDetails = () => {
    // Load character data passed via route loader
    const char = useLoaderData();
    // Local state to store the names of episodes the character appears in
    const [episodeNames, setEpisodeNames] = useState([]);

    // Fetch all episodes the character appears in and update state
    useEffect(() => {
        const getCharacterEpisodes = async () => {
            const characterRes = await fetch(`https://rickandmortyapi.com/api/character/${char.id}`);
            const characterData = await characterRes.json();

            // Extract episode URLs and fetch their names
